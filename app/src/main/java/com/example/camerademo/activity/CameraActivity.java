...
import com.example.camerademo.fragment.ShowInfoDialog;
import com.example.camerademo.rx.Rx;
import com.example.camerademo.utils.BundleArgs;
import com.example.camerademo.utils.Constants;
import com.example.camerademo.utils.GravityManager;
import com.example.camerademo.utils.LOG;
import com.example.camerademo.utils.Picture;
import com.example.camerademo.utils.ScreenUtil;
...
public class CameraActivity extends BaseActivity implements SurfaceHolder.Callback,
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        mFlashItem = menu.add(1, MENU_ITEM_ID_FLASH, Menu.NONE, "闪光灯");
        mFlashItem.setIcon(R.mipmap.flash);
        mFlashItem.setShowAsAction(MenuItem.SHOW_AS_ACTION_ALWAYS);
        mFlashItem.setVisible(CameraManager.get().isSupportFlashLight());
    ...
    public void takePicture() {
        if (!takingPicture) {
            ...
            if (mAutoFocusHandle.focusSuccess || !hasAutoFocusFeature) {
                mAutoFocusHandle.sendEmptyMessage(AutoFocusHandle.FOCUS_FINISH);
            } else {
                getCameraManager().requestAutoFocus(mAutoFocusHandle, AutoFocusHandle.FOCUS_FINISH);
            }
    ...
    protected boolean initCamera(SurfaceHolder surfaceHolder) {
        boolean init = true;
        try {
            CameraManager.get().setPreviewMax(2560);
        ...
        if (!init) {
            ...
            ShowInfoDialog dialog = ShowInfoDialog.newInstance(new ShowInfoDialog.OnClickListener() {
                @Override
            ...
            dialog.show(getSupportFragmentManager(), "ShowInfoDialog");
        }
        return init;
    }
